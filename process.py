...
import pushprocess as papush  # 姿态访问器
import squatprocess as pasquat
import taichi as ti



def read_csv(path):
    nd = np.genfromtxt(path, delimiter=',', skip_header=True)
...
def video_process(video_path, flag):
    ...
    if flag == 1:
        ...
        # TODO::把这些个变成np.array
        left_arm = read_csv('csv/standard/push/push_left_arm.csv')
        right_arm = read_csv('csv/standard/push/push_right_arm.csv')
        left_forearm = read_csv('csv/standard/push/push_left_forearm.csv')
        right_forearm = read_csv('csv/standard/push/push_right_forearm.csv')
    elif flag == 2:
        class_name = 'squat_down'
        out_video_path = './video-output/squat-sample-out-' + mkfile_time + '.mp4'
        body = read_csv('csv/standard/squat/squat_body.csv')
        leg = read_csv('csv/standard/squat/squat_leg.csv')
    ...
    # 初始化poseaccessor
    if flag == 1:
        pose_acc = papush.poseaccessorpush(class_name=class_name, left_arm=left_arm, right_arm=right_arm,
                                           left_forearm=left_forearm, right_forearm=right_forearm)
    elif flag == 2:
        pose_acc = pasquat.poseaccessorsquat(class_name=class_name, body=body, leg=leg)
    ...
    now_count = ti.field(dtype=ti.int32, shape=(1))
    now_count[0] = 1
    frame_idx = 0
    ...
    with tqdm.tqdm(total=video_n_frames, position=0, leave=True) as pbar:
        while True:
            ...
            if (repetitions_count >= 1):
                acc = pose_acc.countif(now_count, repetitions_count, landmarks)
            else:
                acc = 0
